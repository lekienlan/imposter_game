...
import { GameMode, GameState, Phase } from "@imposter/shared";
import { HandleShareInvite } from "../application/HandleShareInvite";
...
const consumeShareInviteParams = () => {
  const url = new URL(window.location.href);
  if (!url.searchParams.has("roomId") && !url.searchParams.has("hostName")) {
    return;
  }
...
export const App = () => {
  ...
  const handleShareInviteUseCase = useMemo(() => new HandleShareInvite(), []);
  ...
  useEffect(() => {
    ...
    if (invite && !inviteHandledRef.current) {
      ...
      const hostLabel = invite.hostName || "host";
      const accepted = window.confirm(`Muốn tham gia game của ${hostLabel} k`);
      consumeShareInviteParams();
      clearSessionState();
      gateway.resetConnection();

      if (accepted) {
        const localPlayerName = localStorage.getItem("playerNameDraft") ?? "";
        const joinPayload = handleShareInviteUseCase.execute(invite, localPlayerName);
        setRoomId(joinPayload.roomId);
        joinRoomUseCase.execute(joinPayload);
      }
      return;
    }
  ...
  }, [handlePhaseUpdate, handleShareInviteUseCase, joinRoomUseCase]);
  ...
  const shareGame = async () => {
    ...
    const hostName = gameState.players.find((player) => player.id === gameState.hostPlayerId)?.name ?? "Host";
    ...
    const shareUrl = buildShareUrl(origin, gameState.roomId, hostName);
    try {
