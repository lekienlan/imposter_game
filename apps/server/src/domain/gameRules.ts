import { GameMode, GameSettings, GameState, Phase, Player, Role, Vote, Winner, WordPair } from "@imposter/shared";
...
const countAliveCitizens = (players: Player[]): number =>
  alivePlayers(players).filter((player) => player.role === Role.CITIZEN || player.role === Role.WHITE).length;
...
const requiredVoterIds = (gameState: GameState): string[] => {
  ...
  if (gameState.settings.mode === GameMode.HARDCORE) {
    return alive.filter((player) => player.role === Role.CITIZEN).map((player) => player.id);
  }
...
export const assignRoles = (players: Player[], settings: GameSettings, random: () => number): Role[] => {
  ...
  if (settings.mode === GameMode.CLASSIC) {
    ...
    return shuffle(
      [...Array(citizenCount).fill(Role.CITIZEN), ...Array(spyCount).fill(Role.SPY), ...Array(whiteCount).fill(Role.WHITE)],
      random
    );
  }
  ...
  return shuffle([...Array(citizenCount).fill(Role.CITIZEN), ...Array(spyCount).fill(Role.SPY)], random);
};
...
export const applyRolesAndWords = (players: Player[], roles: Role[], wordPair: WordPair): Player[] =>
  players.map((player, index) => {
    const role = roles[index];
    const word = role === Role.CITIZEN ? wordPair.citizen : role === Role.SPY ? wordPair.spy : null;
    return { ...player, role, word, statement: null, votedFor: null, isAlive: true };
  });
...
export const beginRoundDescription = (gameState: GameState, random: () => number): void => {
  ...
  if (gameState.round === 1) {
    const whiteIndex = order.findIndex((playerId) => gameState.players.find((player) => player.id === playerId)?.role === Role.WHITE);
...
export const canTransitionToVoting = (gameState: GameState): boolean => gameState.phase === Phase.ROUND_DISCUSSION;
...
export const markStatementSubmitted = (gameState: GameState, playerId: string, statement: string): void => {
...
export const resolveVoting = (gameState: GameState): VoteResolution => {
  ...
  const classicSkipWins = gameState.settings.mode === GameMode.CLASSIC && summary.skipCount > summary.maxVotes;
  if (classicSkipWins || summary.maxVotes === 0) {
    return { status: "SKIP", eliminatedPlayerId: null };
  }

  const tiedWithSkip = gameState.settings.mode === GameMode.CLASSIC && summary.skipCount === summary.maxVotes;
  ...
  if (hasTie) {
    const baseCandidates = gameState.firstRoundTopTargetIds.length > 0 ? gameState.firstRoundTopTargetIds : summary.topTargetIds;
  ...
  return { status: "ELIMINATED", eliminatedPlayerId: summary.topTargetIds[0] ?? null };
};

export const applyRoundResult = (gameState: GameState, eliminatedPlayerId: string | null): Role | null => {
...
const evaluateWinner = (gameState: GameState, eliminatedRole: Role | null): PostRoundOutcome => {
  if (gameState.settings.mode === GameMode.CLASSIC && eliminatedRole === Role.WHITE && gameState.round <= 2) {
    return { winner: Winner.SPIES, winnerReason: "WHITE_ELIMINATED_IN_ROUND_1_OR_2" };
  }
...
const applyWhiteTransition = (gameState: GameState): void => {
  ...
  const whitePlayer = gameState.players.find((player) => player.role === Role.WHITE && player.isAlive);
  if (!whitePlayer) {
    return;
  }
...
export const advanceAfterRound = (gameState: GameState, eliminatedRole: Role | null, random: () => number): void => {
